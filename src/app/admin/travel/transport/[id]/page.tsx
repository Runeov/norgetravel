...
export default function EditTransportPage() {
        ...
        if (json.success && json.data) {
          ...
          if (matched) {
            setForm({
              ...
              transportType: matched.transportType || '',
              operator: matched.operator || '',
              routeFrom: matched.routeFrom || '',
              routeTo: matched.routeTo || '',
              duration: matched.duration != null ? String(matched.duration) : '',
              frequency: matched.frequency || '',
              bookingUrl: matched.bookingUrl || '',
              isEcoFriendly: matched.isEcoFriendly || false,
              seasonalAvailability: matched.seasonalAvailability || '',
            });
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/travel/transport?id=${id}`, {
        method: 'PUT',
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Edit Transport</h1>
          <p className="text-slate-500 text-sm mt-0.5">
            Save Changes for a new transport listing for the travel map.
          </p>
        </div>
      </div>
