...
export default function AdminTransportPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/admin/travel/transport${params.toString() ? `?${params}` : ''}`;
      ...
      console.error('Failed to fetch transport items:', error);
    } finally {
      setLoading(false);
    }
  }, [destination]);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Train className="w-6 h-6 text-[#1B3A5C]" />
            Transport
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Manage transport options — flights, trains, buses, ferries, and more.
          </p>
        </div>
        <Link
          href="/admin/travel/transport/new"
          className={cn(
