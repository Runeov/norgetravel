...
export default function AdminAccommodationPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/admin/travel/accommodation${params.toString() ? `?${params}` : ''}`;
      ...
      console.error('Failed to fetch accommodation items:', error);
    } finally {
      setLoading(false);
    }
  }, [destination]);
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      console.error('Failed to delete accommodation item:', error);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Hotel className="w-6 h-6 text-[#1B3A5C]" />
            Accommodation
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Manage accommodation listings — hotels, cabins, hostels, camping, and more.
          </p>
        </div>
        <Link
          href="/admin/travel/accommodation/new"
          className={cn(
