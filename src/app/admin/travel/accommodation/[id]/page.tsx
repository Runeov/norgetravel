...
export default function EditAccommodationPage() {
    ...
    async function loadItem() {
      try {
        ...
        if (json.success && json.data) {
          const matched = json.data.find((item: any) => item.id === id);
          if (matched) {
            setForm({
              name: matched.name || '',
              description: matched.description || '',
              destination: matched.destination || 'northern-norway',
              location: matched.location || '',
              priceRange: matched.priceRange || 'mid-range',
              website: matched.website || '',
              imageUrl: matched.imageUrl || '',
              imageAlt: matched.imageAlt || '',
              status: matched.status || 'draft',
              isFeatured: matched.isFeatured || false,
              accommodationType: matched.accommodationType || 'hotel',
              starRating: matched.starRating ? String(matched.starRating) : '',
              amenities: Array.isArray(matched.amenities) ? matched.amenities.join(', ') : '',
              capacity: matched.capacity || '',
              checkIn: matched.checkIn || '15:00',
              checkOut: matched.checkOut || '11:00',
              bookingUrl: matched.bookingUrl || '',
              isEcoFriendly: matched.isEcoFriendly || false,
              nearestTown: matched.nearestTown || '',
            });
          } else {
            setError('Item not found');
          }
        } else {
          setError(json.error || 'Failed to load accommodation item');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/travel/accommodation?id=${id}`, {
        method: 'PUT',
      ...
      if (!json.success) {
        setError(json.error || 'Failed to update accommodation item');
        return;
      }

      router.push('/admin/travel/accommodation');
    } catch (err) {
      console.error('Error updating accommodation item:', err);
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Edit Accommodation</h1>
          <p className="text-slate-500 text-sm mt-0.5">
            Modify the accommodation listing for the travel map.
          </p>
        </div>
      </div>
