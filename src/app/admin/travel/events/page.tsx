...
export default function AdminEventsPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/admin/travel/events${params.toString() ? `?${params}` : ''}`;
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      const res = await fetch(`/api/admin/travel/events?id=${id}`, {
        method: 'DELETE',
      });
      const json = await res.json();
      if (json.success) {
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <CalendarDays className="w-6 h-6" style={{ color: navyColor }} />
            Events
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Manage festivals, markets, concerts, and seasonal events across Norway.
          </p>
        </div>
        <Link
          href="/admin/travel/events/new"
          className={cn(
