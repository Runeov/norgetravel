...
export default function AdminRestaurantsPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/admin/travel/restaurants${params.toString() ? `?${params}` : ''}`;
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      console.error('Failed to delete restaurant item:', error);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <UtensilsCrossed className="w-6 h-6 text-[#1B3A5C]" />
            Restaurants
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Manage restaurant listings — Norwegian cuisine, seafood, cafés, fine dining, and more.
          </p>
        </div>
        <Link
          href="/admin/travel/restaurants/new"
          className={cn(
