...
export default function EditRestaurantPage() {
        ...
        if (json.success && json.data) {
          ...
          if (matched) {
            setForm({
              ...
              reservationRequired: matched.reservationRequired || false,
              reservationUrl: matched.reservationUrl || '',
              dietaryOptions: Array.isArray(matched.dietaryOptions) ? matched.dietaryOptions.join(', ') : '',
              averageMealPrice: matched.averageMealPrice || '',
              michelinStars: matched.michelinStars != null ? String(matched.michelinStars) : '',
              specialties: Array.isArray(matched.specialties) ? matched.specialties.join(', ') : '',
              seatingCapacity: matched.seatingCapacity != null ? String(matched.seatingCapacity) : '',
            });
          } else {
            setError('Item not found');
          }
        } else {
          setError(json.error || 'Failed to load item');
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/travel/restaurants?id=${id}`, {
        method: 'PUT',
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            Save Changes for a new restaurant listing for the travel map.
          </p>
        </div>
      </div>
