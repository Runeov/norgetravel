...
export default function AdminExperiencesPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const url = `/api/admin/travel/experiences${params.toString() ? `?${params}` : ''}`;
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      console.error('Failed to delete experience item:', error);
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2">
            <Sparkles className="w-6 h-6" style={{ color: navyColor }} />
            Experiences
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            Manage activities and tours — northern lights, whale watching, dog sledding, and more.
          </p>
        </div>
        <Link
          href="/admin/travel/experiences/new"
          className={cn(
            'flex items-center gap-2 px-4 py-2.5 font-medium rounded-full text-white',
            'hover:shadow-lg transition-all duration-300'
          )}
          style={{ background: `linear-gradient(to right, ${navyColor}, ${greenColor})` }}
        >
